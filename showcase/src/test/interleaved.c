...
void gsTestInterleavedCreate(void) {
	...
	s_pTestInterleavedBfr = simpleBufferCreate(s_pTestInterleavedVPort, WINDOW_SCREEN_WIDTH, WINDOW_SCREEN_HEIGHT);
	
	paletteLoad("data/amidb32.plt", s_pTestInterleavedVPort->pPalette, 1 << WINDOW_SCREEN_BPP);
	bitmapLoadFromFile(s_pTestInterleavedBfr->pBuffer, "data/32c_pal.bm", 0, 0);
